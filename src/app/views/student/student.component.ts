...
export class StudentComponent {
  ...
  refreshData(): void {
    this.apiService.get<ApiResponse>('Student/GetAll').subscribe({
  ...
  onSubmit() {
    ...
    formData.append('relationship', this.currentCountry.relationship);
    //formData.append('logo', this.selectedLogoFile ?? '');
    if (this.selectedLogoFile) {
      formData.append('displayPicture', this.selectedLogoFile);
    }
    formData.append('email', this.currentCountry.email);
    formData.append('parentRelationship', this.currentCountry.parentRelationship);

    formData.append('gender', this.currentCountry.gender);
    formData.append('dob', this.currentCountry.dob);
    formData.append('mobileNumber', this.currentCountry.mobileNumber);
    formData.append('tribe', this.currentCountry.tribe);
    formData.append('religion', this.currentCountry.religion);
    formData.append('parentMobileNumber', this.currentCountry.parentMobileNumber);
    console.log('Form submission successful', formData);

    this.apiService.post<ApiResponse>('Student/Add', formData).subscribe({
      next: (response: any) => {
  ...
  saveCountry() {
    ...
    this.apiService.post<ApiResponse>('Student/Add', this.currentCountry).subscribe({
      next: (response:any) => {
  ...
  updateCountry() {
    ...
    formData.append('religion', this.currentCountry.religion);
    console.log('Form submission successful', formData);
    this.apiService.put<ApiResponse>(`Student/UpdateById/${this.currentCountry.id}`, formData).subscribe({
      next: (response: any) => {
        if (response.status === 'success') {
  ...
  deleteCountry(country: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.apiService.delete<ApiResponse>(`Student/DeleteById/${country.id}`).subscribe({
          next: () => {
