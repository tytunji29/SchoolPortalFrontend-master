...
@Component({
  ...
  templateUrl: './classeslevel.component.html',
  styleUrl: './classeslevel.component.scss'
})
export class ClasseslevelComponent {
  conList: any[] = [];
  searchText: string = '';
  p: number = 1;
  ...
  saveCountry() {
    ...
    this.apiService.post<ApiResponse>('ClassLevel/Add', this.currentCountry).subscribe({
      next: (response:any) => {
        console.log('Class Level added successful', response);
  ...
  updateCountry() {
    ...
    this.apiService.put<ApiResponse>(`ClassLevel/UpdateById/${this.currentCountry._id}`, payload).subscribe({
      next: (response: any) => {
        if (response.status === 'success') {
  ...
  deleteCountry(country: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.apiService.delete<ApiResponse>(`ClassLevel/DeleteById/${country.id}`).subscribe({
          next: () => {
